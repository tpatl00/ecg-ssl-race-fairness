...
class RiskPredictionModel(pl.LightningModule):
    """
    Downstream risk prediction head that attaches to a frozen ECG-FM encoder.

    Architecture:
        ECG-FM Encoder  →  (Batch, 768)
        nn.Sequential   →  (Batch, 1)   [log-risk score for Cox PH loss]

    Args:
        encoder:          Pre-instantiated ECGFM_Encoder (frozen by default).
        model_config:     Dict with optional keys:
                            - hidden_dim (int, default 256)
                            - dropout    (float, default 0.3)
        optimizer_config: Dict with keys:
                            - lr             (float, default 1e-4)
                            - weight_decay   (float, default 1e-5)
                            - warmup_steps   (int,   default 100)
                            - max_steps      (int,   default 1000)
        freeze_encoder:   If True (default), encoder weights are frozen.
    """
    def __init__(
            self,
            # encoder,
        ...
        # if self.hparams.model_config['freeze_encoder']:
        #     self.encoder.eval()
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor = None) -> torch.Tensor:
        # """
        # Args:
        #     x:    (Batch, 12, 5000) raw ECG waveform.
        #     mask: Optional padding mask passed through to the encoder.
        # Returns:
        ...
        # latent = self.encoder(x, mask=mask)   # (Batch, 768)
        latent = x
    ...
    def _common_step(self, batch: dict, stage: str) -> dict:
        ...
        Expects batch keys:
            'ecg_waveform'         – (Batch, 12, 5000)  raw waveform
            'eventStatus'          – (Batch,)            0/1 event indicator
            'followUpTime'         – (Batch,)            follow-up duration
        """
        ...
        if batch_idx := 0 or True:  # always-on cheap stats
            with torch.no_grad():
                self.log(f"{stage}/logrisk_std",  log_risk.detach().float().std(),  on_step=True, on_epoch=False)
                self.log(f"{stage}/logrisk_mean", log_risk.detach().float().mean(), on_step=True, on_epoch=False)
    ...
    def _on_epoch_end(self, outputs: list, stage: str):
        ...
        # so we negate log_risk to align sign convention.
        try:
    ...
    def configure_optimizers(self):
        ...
        # Only optimise parameters that require gradients (encoder is frozen)
        trainable = [p for p in self.parameters() if p.requires_grad]
        optimizer = torch.optim.AdamW(trainable, lr=lr, weight_decay=weight_decay)

        # Cosine annealing with linear warmup via LambdaLR -> Dr Chen's work
